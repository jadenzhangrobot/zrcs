...
ZRCS GUI 2.0 采用专业的深色主题设计，符合工业现场的高对比度需求。界面分为 7 大功能模块，提供完整的运动控制解决方案。
...
A: 编辑 `zrcsStyles.h` 中的 Colors 命名空间，或编辑 `mainwindow_refactored.cpp` 中的 `setupStyles()` 函数。

**Q: 如何添加新的轴?**

A: 在 `zrcsConfig.h` 中修改 `axisCount`，并在 `JogControlPanel::setupUI()` 中确认按钮自动生成逻辑。
...
A: 修改 `onHomeRequested()` 和 `onHomeAllRequested()` 中的确认对话框调用。

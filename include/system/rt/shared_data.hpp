...
struct systemRegister {
    std::atomic<std::array<bool, 100>> sysControl; // 系统控制寄存器
    std::atomic<std::array<bool, 100>> sysStatus; // 系统状态寄存器  
};
struct SharedBlock {
    // NRT -> RT 的命令通道
...
typedef std::atomic<uint64_t>  heartBeat;  // 心跳寄存器
// ===================================================================
// 2. 单生产者、单消费者（SPSC）无锁环形缓冲区模板
// ===================================================================

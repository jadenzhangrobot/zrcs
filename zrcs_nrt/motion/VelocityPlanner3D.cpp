...
VelocityPlanner3D::VelocityPlanner3D() : max_vel_global_(100.0), max_accel_(100.0), max_jerk_(1000.0), start_vel_(0.0), end_vel_(0.0), corner_tolerance_(0.5)
{
    }

void VelocityPlanner3D::setConfig(double max_v, double max_a, double start_v, double end_v, double corner_tol, double max_j)
{
...
bool VelocityPlanner3D::planSegments(std::vector<TrajectorySegment>& segments)
{
        if (segments.empty()) {
...
double VelocityPlanner3D::calculateLocalVelocityLimit(const TrajectorySegment& segment) const
{
...
double VelocityPlanner3D::calculateJunctionVelocityLimit(const TrajectorySegment& prev, const TrajectorySegment& next) const
{
...
double VelocityPlanner3D::transitionTime(double v0, double v1) const
{
...
double VelocityPlanner3D::dMinTransition(double v0, double v1) const
{
        return 0.5 * (v0 + v1) * transitionTime(v0, v1);
    }

double VelocityPlanner3D::maxReachableVel(double v0, double distance) const
{
        double lo = v0;
...
double VelocityPlanner3D::estimateSegmentDuration(const TrajectorySegment& segment) const
{

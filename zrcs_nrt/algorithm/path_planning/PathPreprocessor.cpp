...
std::vector<TrajectorySegment> PathPreprocessor::fitCornerBlendSegments( const std::vector<PathMoveBlock>& blocks, double cornerTol)
{
        ...
        std::vector<PathPreprocessor::CornerBlend> corners(N);
        if (N > 2) {
...
Point3D PathPreprocessor::evaluate(const TrajectorySegment& segment, double u)
{
        return evaluateSegment(segment, u);
    }

Point3D PathPreprocessor::tangent(const TrajectorySegment& segment, double u)
{
        return segmentTangent(segment, u);
    }

double PathPreprocessor::curvature(const TrajectorySegment& segment, double u)
{
        return segmentCurvature(segment, u);
    }

void PathPreprocessor::buildCornerBlends(const std::vector<Point3D>& raw, double cornerTol, std::vector<PathPreprocessor::CornerBlend>& corners)
{
...
TrajectorySegment PathPreprocessor::makeLineSegment(int segmentId, int sourceBlockId, const Point3D& start, const Point3D& end, double rx, double ry, double rz, double feedrate)
{
...
TrajectorySegment PathPreprocessor::makeArcSegment(int segmentId, int sourceBlockId, const PathPreprocessor::CornerBlend& corner, double rx, double ry, double rz, double feedrate)
{
        TrajectorySegment segment;
        segment.segment_id = segmentId;
        segment.source_block_id = sourceBlockId;
...
TrajectorySegment PathPreprocessor::makeCubicSpanSegment(int segmentId, int sourceBlockId, const std::array<Point3D, 5>& ctrl, double u0, double u1, double rx, double ry, double rz, double feedrate)
{
...
void PathPreprocessor::setAxis(TrajectorySegment& segment, int axis, double c0, double c1, double c2, double c3)
{
...
std::array<double, 4> PathPreprocessor::fitCubicFromSamples(double y0, double y1, double y2, double y3)
{
        double m[3][4] = {
...
void PathPreprocessor::finalizeGeometry(TrajectorySegment& segment)
{
        segment.length = estimateLength(segment);
        segment.max_curvature = estimateMaxCurvature(segment);
    }

double PathPreprocessor::estimateLength(const TrajectorySegment& segment)
{
...
double PathPreprocessor::integrateSegmentSpeed(const TrajectorySegment& segment, double u)
{
...
double PathPreprocessor::estimateMaxCurvature(const TrajectorySegment& segment)
{
        if (segment.type == TrajectorySegmentType::Line) {
            return 0.0;
        }
...
double PathPreprocessor::mergeFeedrate(double a, double b)
{
        if (a > 0.0 && b > 0.0) {
            return std::min(a, b);
        }
...
Point3D PathPreprocessor::evalClampedCubicBSpline(const std::array<Point3D, 5>& ctrl, double u)
{
        if (u <= 0.0) {
            return ctrl.front();
        }
